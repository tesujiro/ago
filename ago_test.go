	...
	sections := []struct {
	...
	}{
		{
			...
			tests: []test{
				...
				{script: `BEGIN{print 5**2}`, ok: "25\n"},
				{script: `BEGIN{print 5^2}`, ok: "25\n"},
				{script: `BEGIN{print 3*10**2/10}`, ok: "30\n"},
		...
		},
		{
			...
			tests: []test{
				...
				{script: `BEGIN{a=10;a^=4;print a}`, ok: "10000\n"},
				{script: `BEGIN{a=10;a**=4;print a}`, ok: "10000\n"},
		...
		*}`, okRegex: "Syntax error: syntax error", rc: 1},
		...
		},
		{
			name: "Local Variable",
			tests: []test{
				{prepare: func() { os.Args = []string{os.Args[0], "-g=false"} }, script: `BEGIN{l=1}END{print l}`, ok: "\n"},
				{prepare: func() { os.Args = []string{os.Args[0], "-g=false"} },
		...
		},
		{
			...
			tests: []test{
				...
				{script: `BEGIN{a=0;while (a<=10) { a= a+1 };print a}`, ok: "11\n"},
				//{script: "BEGIN{a=0;while (a<=10) \na= a+1 ;print a}", ok: "11\n"},
				//{script: "BEGIN{a=0;while (a<=10) a=a+1;\nprint a\n}", ok: "11\n"},
		...
		},
		{
			...
			tests: []test{
				...
				{script: `BEGIN{a[1]=1;a[2]=2;for (i in a) {};print i}`, ok: "2\n"},
		...
		{
			name: "Higher Order & Recursive Functions",
			tests: []test{
				{
					prepare: func() { os.Args = []string{os.Args[0], "-g=false"} },
					script:  `BEGIN{ mod=func(x){ f=func(y){ if y<x {return y} else { return f(y-x) } }; return f }; mod3=mod(3); print mod3(11);}`,
					ok:      "2\n",
				},
				{prepare: func() { os.Args = []string{os.Args[0], "-g=false"} },
